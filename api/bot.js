...
const AUTH_FILE = '/tmp/authorized.json'; // Vercel 可写目录（/tmp）
...
async function loadAuth() {
  try {
    ...
    authorizedUsers = new Map(JSON.parse(data));
    console.log('✅ 授权数据加载成功');
  } catch (error) {
...
async function saveAuth() {
  try {
    await fs.writeFile(AUTH_FILE, JSON.stringify(Object.fromEntries(authorizedUsers)));
    console.log('💾 授权数据保存成功');
  } catch (error) {
    console.error('❌ 保存授权失败:', error);
  }
}
await loadAuth(); // 启动时加载
...
bot.command('qc', async (ctx) => {
  ...
  let startMessageId = ctx.message.message_id; // 默认从当前消息开始
  const replyTo = ctx.message.reply_to_message;
  const maxDelete = 1000; // 最大删除条数
  if (replyTo) {
    startMessageId = replyTo.message_id;
    ctx.reply(`🗑️ 开始清除从消息 ID ${startMessageId} 往上 ${maxDelete} 条记录...`);
  } else {
    ctx.reply(`🗑️ 开始清除群聊最近 ${maxDelete} 条记录... `);
  }
  let deletedCount = 0;
  // 分页删除：每批100条
  for (let batch = 0; batch < Math.ceil(maxDelete / 100); batch++) {
    const batchStart = startMessageId - (batch * 100);
    const ids = [];
    for (let i = 0; i < 100 && (batchStart - i) > 0; i++) {
    ...
    for (const msgId of ids) {
      try {
        ...
        deletedCount++;
        await new Promise(resolve => setTimeout(resolve, 50)); // 50ms 延迟防限流
      } catch (error) {
...
module.exports = async (req, res) => {
  ...
  res.status(200).json({
    ...
    authorizedCount: authorizedUsers.size
  });
};